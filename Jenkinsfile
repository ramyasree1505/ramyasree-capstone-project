        ...
        stage('Deploy to EC2') {
            steps {
                bat """
                    ...
                    "docker rm -f node-app || true && docker pull ramyasree15/my-capstone-project:latest && docker run -d --name node-app -p 3000:3000 ramyasree15/my-capstone-project:latest"
                """
            }
        }
    }
